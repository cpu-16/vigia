...
with sync_playwright() as p:
 b=p.chromium.launch(headless=True,args=['--no-sandbox']);ctx=b.new_context(viewport={'width':390,'height':844},service_workers='block');page=ctx.new_page();errors=[];page.on('pageerror',lambda e:errors.append(str(e)));state={'extracts':0,'saved':None}
 ...
 if os.environ.get('PROBAR_CORRECCION'):
  page.locator('#volver').click();page.locator('#texto').fill('Dos resonadores en Hospital DemoCare Horizon');page.locator('#accion').click();page.wait_for_function("document.querySelector('#filas').textContent.includes('Hospital DemoCare Horizon')")
 ...
 print(json.dumps({'foto_durante_preguntas':True,'respuestas_conservadas':True,'cantidad_total':2,'inferencias':state['extracts'],'guardado_con_pendientes':True,'corregir_sin_perder_unidades':bool(os.environ.get('PROBAR_CORRECCION')),'errores_js':errors},ensure_ascii=False));b.close()
server.shutdown()
